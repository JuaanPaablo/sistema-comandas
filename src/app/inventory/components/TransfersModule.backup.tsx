...
export default function TransfersModule({ onOpenModal, onOpenConfirmationModal, onCloseModal }: TransfersModuleProps) {
  ...
  const openModal = (transfer?: Transfer) => {
    ...
    if (onOpenModal) {
      onOpenModal('transfer', editingTransfer ? 'Editar Transferencia' : 'Nueva Transferencia', null);
    } else {
      setShowModal(true);
    }
  };
  ...
  const onSubmit = useCallback(async (data: TransferFormData) => {
    const buttonId = 'create-transfer-modal';
    try {
      const transferData = { ...data };
      let response;
      if (editingTransfer) {
        response = await TransferService.update(editingTransfer.id, transferData);
        
        // Si se está marcando como completada, actualizar stock
        if (transferData.status === 'completed' && editingTransfer.status !== 'completed') {
          console.log('📊 Actualizando stock por transferencia completada...');
          await updateStockOnTransferCompletion(response.data, allCategories);
        }
      } else {
        response = await TransferService.create(transferData);
  ...
  }, [editingTransfer, onCloseModal, loadData, setButtonLoading, allCategories, updateStockOnTransferCompletion]);
  ...
  const handleDelete = async (id: string) => {
    if (onOpenConfirmationModal) {
      onOpenConfirmationModal(
        ...
        'Esta acción colocará la transferencia como inactiva. Podrás reactivarla después.',
        async () => {
          try {
  ...
  const handleHardDelete = async (id: string) => {
    if (onOpenConfirmationModal) {
      onOpenConfirmationModal(
        'Eliminar Transferencia Permanentemente',
        '⚠️ Esta acción NO se puede deshacer. Se eliminará definitivamente la transferencia.',
        async () => {
          try {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button 
              ref={submitButtonRef}
              type="submit" 
              disabled={isSubmitting || isButtonLoading('create-transfer-modal') || modalButtonLoading}
              onClick={(e) => {
                // DESHABILITAR INMEDIATAMENTE AL HACER CLIC
                if (!isSubmitting && !isButtonLoading('create-transfer-modal') && !modalButtonLoading) {
                  ...
                  setIsSubmitting(true);
                  setButtonLoading('create-transfer-modal', true);
            ...
            >
              {isSubmitting || isButtonLoading('create-transfer-modal') || modalButtonLoading ? (
                <div className="flex items-center space-x-2">
                  <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div>
                  <span>{editingTransfer ? 'Actualizando...' : 'Creando...'}</span>
                </div>
              ) : (
